...
新增可搜尋篇章書架、三關暖身、選線索與作答把握、答後反思、待請教題目、大字閱讀及十分鐘無裝置教學指引。房間狀態未變時不重畫介面；程式拆分為學習、練習、房間與教師功能模組。

閱讀 [4.0 升級與教學設計](docs/4.0升級與教學設計.md)。正式入口以 GitHub play.html 為準；Google 後端相容第62版，無須搬移學生資料。
...
- 新增六款無需打字的題型：配對、排序、分類、點圖、是非滑卡、刪選。
- 教師可用表單編題、試玩預覽及加入私人題庫，無需輸入 JSON。
- 互動題接上個人練習、投影課堂及多人房間，Google 後端負責計分。
- 修正中文識別碼的編碼碰撞，明確使用 UTF-8；保留原有題目。
- 古紙、墨色、木框與銅金旗幟介面；動畫可關閉，觸控拖曳均有按鈕替代操作。
...
GitHub Pages 不執行 Google Apps Script。正式遊戲透過 Google 隱藏通訊頁及 postMessage 傳送請求，驗證來源網址、通道及回應視窗；教師權限仍由後端驗證。把 GitHub 原始碼更新，不會自動改寫 Google 部署。部署者須依 [部署指南](docs/DEPLOYMENT.md) 完成兩邊更新；教師日常使用請看 [快速開始](docs/教師快速開始.md)。
...
有裝置：學生可在正式 GitHub 遊戲加入房間，或課後個人研習。不要把試玩頁當作正式交功課入口。
