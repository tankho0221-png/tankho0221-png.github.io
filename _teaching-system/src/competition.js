...
function classroomReadiness(){
  ...
  return `<div class="class-readiness">${marchGraphic(done/game.teams.length*100)}<p><b>本輪 ${done} / ${game.teams.length} 組已亮板</b> · 教師手動標記</p><p class="muted">每人先選答案，再同時亮板；此標記不計分，也不代表答對。</p><div class="toolbar">${game.teams.map((t,i)=>`<button class="btn ${ready[i]?'primary':'ghost'}" data-ready="${i}" aria-pressed="${!!ready[i]}">${esc(t.name)} · ${ready[i]?'已亮板 ✓':'待亮板'}</button>`).join('')}</div></div>`;
}
