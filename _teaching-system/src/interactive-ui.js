...
function renderInteractive(root,q,opts={}) {
  ...
  const state=(opts.revealed&&opts.draft?JSON.parse(JSON.stringify(opts.draft)):opts.draft)||{values:q.kind==='order'?shuffle(p.items.map((_,i)=>i),(opts.seed||1)+13):p.items.map(()=>null),choice:null,removed:[],selected:null};
  ...
  function draw(){
    ...
    if(q.kind==='match'||q.kind==='classify'){
      html+='<p class="iq-instruction">先選一張卡，再點目的地；電腦可直接拖整張卡，觸控可拖「移動」把手。</p><div class="iq-pairs"><div class="iq-items">';
      html+=p.items.map((s,i)=>button(`<span>${esc(s)}</span><small>${state.values[i]===null?'待安排':'→ '+esc(p.targets[state.values[i]])}</small>${disabled?'':`<span class="iq-handle" data-drag="${i}" aria-hidden="true">⠿ 移動</span>`}`,`data-item="${i}" aria-pressed="${state.selected===i}"`)).join('');
    ...
    if(one('[data-submit]'))one('[data-submit]').disabled=!IQ.validAnswer(q,value());
    const changed=()=>{draw();if(opts.onChange)opts.onChange();};
    function assign(i,j){
      if(q.kind==='match'){const previous=state.values.indexOf(j);if(previous>=0&&previous!==i)state.values[previous]=null;}
      state.values[i]=j;state.selected=null;changed();
    }
    ...
    const move=(from,to)=>{if(to<0||to>=state.values.length||from===to)return;const [item]=state.values.splice(from,1);state.values.splice(to,0,item);changed();};
    ...
    all('[data-choice-index]').forEach(b=>b.onclick=()=>{state.choice=Number(b.dataset.choiceIndex);changed();});
    all('[data-binary]').forEach(b=>b.onclick=()=>{state.choice=b.dataset.binary==='true';changed();});
    all('[data-remove]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.remove),idx=state.removed.indexOf(i);if(idx>=0)state.removed.splice(idx,1);else if(state.removed.length<3)state.removed.push(i);changed();});
    // Native desktop drag supports the whole card, with a local source guard.
    let dragged=null;
    all(q.kind==='order'?'.iq-order button[data-position]':'[data-item]').forEach(tile=>{
      tile.draggable=true;
      tile.ondragstart=e=>{dragged=Number(q.kind==='order'?tile.dataset.position:tile.dataset.item);e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',String(dragged));tile.classList.add('dragging');};
      tile.ondragend=()=>{dragged=null;tile.classList.remove('dragging');all('.iq-drop-active').forEach(t=>t.classList.remove('iq-drop-active'));};
    });
    all(q.kind==='order'?'.iq-order li[data-position]':'[data-target]').forEach(target=>{
      target.ondragover=e=>{if(dragged===null)return;e.preventDefault();e.dataTransfer.dropEffect='move';target.classList.add('iq-drop-active');};
      target.ondragleave=()=>target.classList.remove('iq-drop-active');
      target.ondrop=e=>{if(dragged===null)return;e.preventDefault();const from=dragged;dragged=null;if(q.kind==='order')move(from,Number(target.dataset.position));else assign(from,Number(target.dataset.target));};
    });
    all('[data-drag]').forEach(handle=>{
      handle.onpointerdown=e=>{if(e.button!==0||e.pointerType==='mouse')return;e.preventDefault();e.stopPropagation();const start=Number(handle.dataset.drag);handle.setPointerCapture(e.pointerId);handle.classList.add('dragging');handle.onpointerup=up=>{handle.releasePointerCapture(up.pointerId);const target=document.elementFromPoint(up.clientX,up.clientY);handle.classList.remove('dragging');if(q.kind==='order'){const drop=target?.closest('[data-position]');if(drop&&root.contains(drop))move(start,Number(drop.dataset.position));}else{const drop=target?.closest('[data-target]');if(drop&&root.contains(drop))assign(start,Number(drop.dataset.target));}};handle.onpointercancel=()=>handle.classList.remove('dragging');};
    });
    const decision=one('.iq-decision');if(decision){let start=null;decision.onpointerdown=e=>{if(e.target.closest('button'))return;start=[e.clientX,e.clientY];decision.setPointerCapture(e.pointerId);};decision.onpointerup=e=>{if(!start)return;const dx=e.clientX-start[0],dy=e.clientY-start[1];start=null;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)*1.5){state.choice=dx>0;changed();}};decision.onpointercancel=()=>{start=null;};decision.onkeydown=e=>{if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();state.choice=e.key==='ArrowRight';changed();}};}
    if(one('[data-reset]'))one('[data-reset]').onclick=()=>{state.values=q.kind==='order'?p.items.map((_,i)=>i):p.items.map(()=>null);state.choice=null;state.removed=[];state.selected=null;draw();};
    if(one('[data-submit]'))one('[data-submit]').onclick=async()=>{const v=value();if(!IQ.validAnswer(q,v))return;const b=one('[data-submit]');b.disabled=true;try{await opts.onSubmit(v);}catch(e){toast(e.message);}finally{if(root.contains(b))b.disabled=false;}};
  }
  draw();return state;
}
function mountLiveInteractive(r){
  ...
  const next=renderInteractive(root,q,{draft,disabled:r.isHost||r.state.phase!=='answer'||r.submitted,readOnlyMessage:r.isHost?'主持人總覽：請由學生在自己的裝置作答。':r.submitted?'軍報已提交，請等候揭曉。':'等待老師開放作答。',revealed:r.state.phase==='reveal',seed:Number(live.pin),onSubmit:async value=>{await rpc('submitLiveAnswer',live.pin,live.token,r.state.index,value);toast('軍報已提交，等候揭曉。');await refreshLive();}});
